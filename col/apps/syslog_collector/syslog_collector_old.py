...
from pylib import conf, homing, logger, wiring, msgfilling, inet
...
def _handle_data(data, sid, parser, syslog_out, device_name, ip, collected_at):
    syslog_out.start_benchmarker_processing()
    ...
    for event in parser:
        ...
        syslog_out.send_with_mid(event)
...
def _handle_tcp_client(sock, addr, config, syslog_out):
    ...
    try:
        ...
        while True:
            ...
            _handle_data(data, sid, parser, syslog_out, profile["device_name"], ip, config["loginspect_name"])
    except Exception, e:
        log.warn('Exception receiving message: %s' % str(e))
    finally:
        sock.close()


def start_tcp_server(port, config, syslog_out):
    log.info("Syslog Collector; listening tcp server at %s", port)

    def handler(sock, addr):
        return _handle_tcp_client(sock, addr, config, syslog_out)
...
def start_ssl_enabled_tcp_server(port, config, syslog_out, ssl_args):
    log.info("Syslog Collector; listening ssl enabled tcp server at %s", port)

    def handler(sock, addr):
        log.info("Handling ssl enabled client from %s", port)
        return _handle_tcp_client(sock, addr, config, syslog_out)

    listener = _create_listener(port)
    tcp_server = StreamServer(listener, handler, keyfile=ssl_args['keyfile'],
                              certfile=ssl_args['certfile'])
    tcp_server.start()
...
def start_udp_server(port, config, syslog_out):
    log.info("Syslog Collector; listening udp server at %s", port)
    ...
    while True:
        ...
        _handle_data(data, sid, parser, syslog_out, device_name, ip, collected_at)
...
def main():
    ...
    syslog_ssl = False
    if config.get("ssl_port"):
        ssl_port = config["ssl_port"]
        ssl_args = homing.homize(config["ssl_args"])
        syslog_ssl = True
    ...
    start_tcp_server(port, config, syslog_out)
    if syslog_ssl:
        start_ssl_enabled_tcp_server(ssl_port, config, syslog_out, ssl_args)
    start_udp_server(port, config, syslog_out)
