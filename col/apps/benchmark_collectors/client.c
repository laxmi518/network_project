...
void* do_connect(struct thread_data_t *td)
{
     struct addrinfo *servinfo = td->ai;
    ...
    for (i = 0; i < g_nloop; ++i) {
        ...
        clock_gettime(CLOCK_MONOTONIC, &t1);
            ...
            for (k=0; k<g_noverwrap; ++k) {
                sockfd = socks[k];
                char *msg;
                asprintf(&msg,"<23> Mar 29 2004 09:56:39 abc \n%d",i);
                // asprintf(&msg,"hello\n");
...
int main(int argc, char *argv[])
{
    ...
    long long time_consumed;
    {
        ...
        for (i = 0; i < nthread; ++i) {
            ...
            td->ai=servinfo;
            td->thread_no = i;
            rv = pthread_create(&threads[i], NULL, (void*)do_connect, (void*)td);
            if (rv == -1) {
