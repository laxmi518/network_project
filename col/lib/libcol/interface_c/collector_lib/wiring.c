...
extern int _cnt;             /**< Log counter */
extern GMutex _mutex_cnt;    /**< Mutex for counter */
extern GMutex _mutex_socket; /**< Mutex for socket */
...
#ifdef BENCHMARK
        g_mutex_lock (&_mutex_cnt);
        _cnt += 1;
        if(_cnt % 1000 == 0)
        {
            struct timeval end_time;
            gettimeofday(&end_time, NULL);
            printf("benchmark data: count=%u time= ", _cnt);
            timeval_print(&end_time);
        }
        g_mutex_unlock(&_mutex_cnt);
#endif
    char *event_st;
