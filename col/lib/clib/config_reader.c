...
json_t *get_json_from_config_file(char *config_path) {
    ...
    if(!config)
    {
        zlog_error(c,"Error reading config: on line %d: %s\n", error.line, error.text);
        exit(0);
    }
...
const char *get_string_value_from_json(json_t *root, const char *field)
{
	...
	{
		zlog_error(c,"Error parsing json object: value of %s field is not a string\n", field);
		exit(0);
	}
...
int get_integer_value_from_json(json_t *root, char *field)
{
	...
	{
		zlog_error(c,"Error parsing json object: value of %s field is not an integer\n", field);
		exit(0);
	}
...
json_t *get_json_object_from_json(json_t *root, char *field)
{
	...
	{
		zlog_error(c,"Error parsing json object: value of %s is not a json object\n", field);
		return NULL;
	}
